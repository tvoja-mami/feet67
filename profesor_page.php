...
if (isset($_POST['delete_grade'])) {
    ...
    $delete_sql = "DELETE FROM oddaje WHERE id = $ocena_id";
...
if (isset($_POST['submit_grade'])) {
    $ucenec_id = (int)$_POST['ucenec_id'];
    $predmet_id = (int)$_POST['predmet_id'];
    $ocena = (int)$_POST['ocena'];

    $sql = "INSERT INTO oddaje (id_ucenec, id_naloga, ocena, oddano_ob) VALUES ($ucenec_id, $predmet_id, $ocena, NOW())";
...
$students_query = "SELECT id, ime, priimek FROM uporabniki WHERE vloga = 'ucenec'";
$students_result = $conn->query($students_query);

// Get list of assignments for this teacher
$id_ucitelj = $_SESSION['user_id'];
$naloge_query = "
    SELECT n.*, p.ime as ime_predmeta
    FROM naloge n
    JOIN predmeti p ON n.id_predmet = p.id
    JOIN ucitelji_predmeti up ON p.id = up.id_predmet
    WHERE up.id_ucitelj = $id_ucitelj
    ORDER BY p.ime, n.naslov";
...
<body style="background: #f5f7fb;">
    <div class="box">
            ...
            <form method="POST">
                    ...
                    <select name="ucenec_id" required>
                        ...
                        <?php while($student = $students_result->fetch_assoc()): ?>
                            <option value="<?= htmlspecialchars($student['id']) ?>">
                                <?= htmlspecialchars($student['ime'] . ' ' . $student['priimek']) ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="predmet_id" required>
                        <option value="">Izberi nalogo</option>
                        <?php while($naloga = $naloge_result->fetch_assoc()): ?>
                            <option value="<?= htmlspecialchars($naloga['id']) ?>">
                                <?= htmlspecialchars($naloga['ime_predmeta'] . ' - ' . $naloga['naslov']) ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
                </div>
        ...
        <?php
        $grades_query = "SELECT o.*, u.ime, u.priimek, n.naslov as naslov_naloge 
                        FROM oddaje o 
            ...
            <table class="grades-table">
                ...
                <tbody>
                    <?php while($grade = $grades_result->fetch_assoc()): ?>
                        <tr>
                            <td><?= htmlspecialchars($grade['ime'] . ' ' . $grade['priimek']) ?></td>
                            <td><?= htmlspecialchars($grade['naslov_naloge']) ?></td>
                            <td><?= htmlspecialchars($grade['ocena']) ?></td>
                            <td><?= htmlspecialchars(date('d.m.Y', strtotime($grade['oddano_ob']))) ?></td>
                            <td>
